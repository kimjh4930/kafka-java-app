    ...
    private void save(int partitionNo) {
        if (bufferString.get(partitionNo).size() > 0)
            try {
                ...
                configuration.set("fs.defaultFS", "hdfs://192.168.0.73:9000");
                FileSystem hdfsFileSystem = FileSystem.get(configuration);
                FSDataOutputStream fileOutputStream = hdfsFileSystem.create(new
                        Path(fileName));
                fileOutputStream.writeBytes(StringUtils.join(bufferString.
                        get(partitionNo), "\n"));
